...
(() => {
  // ===== 1) Presets: (Masculine|Feminine|Neutral) × (Warm|Neutral|Night)
  const NAR_PRESETS = {
    Masculine: {
  ...
  function scoreVoice(v, gender) {
    ...
    const masculineHints = [
      'google uk english male','google us english','daniel','paul','thomas',
      'alex','henry','oliver','george','charlie','liam','noah'
  ...
  function mountNarratorControls() {
    ...
    const radios = Array.from(document.querySelectorAll('input[name="voice"]')); // warm|neutral|night

    // --- New controls (selects + bouton data-narrator-play)
    const btnPlay = root?.querySelector('[data-narrator-play]');
    const selType = root?.querySelector('#nar-voice-type'); // Masculine|Feminine|Neutral
    const selTone = root?.querySelector('#nar-tone');       // Warm|Neutral|Night
